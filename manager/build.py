...
def build_content(output='output'):
    ...
    build_manager.render_templates()
    build_manager.make_cname()
    build_manager.copy_static()
...
class BuildManager():
    ...
    def __init__(self, output='output', content='content'):
        self.output = output
    ...
    def copy_static(self):
        source = os.path.abspath(os.path.join(self.content, 'static'))
        destination = os.path.abspath(os.path.join(self.output, 'static'))
        shutil.copytree(source, destination)

    def render_templates(self):
        root = os.path.join(self.content, 'templates')
        root = os.path.abspath(root)
        paths = self.search_pattern(root, '*.htm')
        paths += self.search_pattern(root, '*.html')
        file_loader = FileSystemLoader(root, encoding='utf-8', followlinks=True)
        env = Environment(loader=file_loader)
        for path in paths:
            basename = path.replace(root, '').strip('\\')
            template = env.get_template(basename)
            output = template.render()
            output_path = os.path.join(self.output, basename)
            sys.stdout.flush()
